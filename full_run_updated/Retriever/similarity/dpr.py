...
class DPRRetriever(BaseRetriever):
    ...
    def embed_texts(self, texts, text_type):
        """DPR uses pooler_output (CLS token) for embedding extraction."""
        if text_type == 'queries':
            return create_embeddings(self.qtokenizer, self.qmodel, texts, method="pooler_output")
        if text_type == 'corpus':
            return create_embeddings(self.ctokenizer, self.cmodel, texts, method="pooler_output")
